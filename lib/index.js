...
function logEvent(ctx, data, request) {
  ...
  // added ability to log the hashedid that gets sent to the client in the X-Request-Id header.
  if (request) {
    obj.req_id = request.id;
    if (request.app && request.app.hashedid) {
      obj.Hreq_id = request.app.hashedid;
    }
  }
